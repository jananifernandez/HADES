...
void PluginEditor::paint (juce::Graphics& g)
{
    ...
    switch (currentWarning){
        ...
        case k_warning_frameSize:
            ...
            g.drawText(TRANS("Set host block size to \"") + String(hades_renderer_getFrameSize()) + TRANS("\" for lowest latency"),
                       getBounds().getWidth()-225, 16, 530, 11,
                       Justification::centredLeft, true);
            break;
    }
...
void PluginEditor::timerCallback()
{
    ...
    if ( !((hades_renderer_getDAWsamplerate(hHdR) == 44.1e3) || (hades_renderer_getDAWsamplerate(hHdR) == 48e3)) ){
    ...
    else if ((processor.getCurrentBlockSize() != hades_renderer_getFrameSize())){
